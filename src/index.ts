...
export interface ObfuscatorBuildOptions extends Omit<BuildConfig, "plugins"> {
  ...
  /**
   * Skip generating the vendor bundle but still rewrite imports to use an existing vendor bundle.
   * Useful when building multiple entrypoints that should share the same vendor bundle.
   * The first build should generate the vendor bundle (skipVendorBundleGeneration: false),
   * and subsequent builds should reuse it (skipVendorBundleGeneration: true).
   * Defaults to false.
   */
  skipVendorBundleGeneration?: boolean;

  /**
   * Path to an existing vendor bundle to use for import rewriting.
   * Required when skipVendorBundleGeneration is true.
   * This should be the path to the vendor.js file generated by a previous build.
   * The function will read this file to extract the dependency mapping.
   */
  existingVendorBundlePath?: string;
}
...
export async function obfuscatedBuild(options: ObfuscatorBuildOptions): Promise<ObfuscatorBuildResult> {
  const {
    ...
    skipVendorBundleGeneration = false,
  ...
  if (bundleNodeModules && externalArray.length > 0) {
    ...
    // Helper function to rewrite imports in a file to use the vendor bundle
    const rewriteImportsToUseVendor = (filePath: string, vendorBundlePath: string) => {
    ...
    if (skipVendorBundleGeneration) {
      // Skip vendor bundle generation, but still rewrite imports to use the existing vendor bundle
      const vendorBundlePath = "./" + nodeModulesBundleName;

      // Verify the vendor bundle exists if a path was provided
      if (existingVendorBundlePath && !existsSync(existingVendorBundlePath)) {
        console.warn(`[bun-plugin-javascript-obfuscator] Warning: Existing vendor bundle not found at ${existingVendorBundlePath}`);
      }

      for (const output of obfuscatedOutputs) {
        if (output.path.endsWith(".js") || output.path.endsWith(".mjs")) {
          rewriteImportsToUseVendor(output.path, vendorBundlePath);
        }
      }
    } else {
